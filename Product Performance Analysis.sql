...
select * from website_sessions;
  # Traffic source
Select * from website_sessions;
# where the bulk of our website coming from through yesterday
...
FROM
 ...
 #calculate conversion rate from sessions to order
...
#3 finding weekly session per week
select
-- year(created_at) as yearly,
date_format(min(created_at),'%m/%d/%Y') as week_start_date,
...
#4 Pulling conversation rate from session to order for device_type
select
...
#5 Pulling weekly trend for both desktop and mobile
select
-- week(created_at),
date_format(min(created_at),'%m/%d/%Y') as week_start_date,
count(distinct case when device_type = 'desktop' then website_session_id else null end) as desk_session,
...
# 1 The most view website pag rankrd by sessions
...
#2 Pulling Top Entry page
create temporary table Landing
select min(website_pageview_id) as entry,
	    website_session_id
        ...
        select 
        count(Landing.website_session_id) as Top_entry,
        website_pageviews.pageview_url
        from
        Landing 
        left join
        website_pageviews on Landing.website_session_id = website_pageviews.website_session_id
        group by 2;
        
     #3 Bounce rate for traffic landing on the Home page
# First get the first website_pageview for relevant sessions
...
FROM
    ...
    group by 2;
   # identify the landing page of each of the website the website_pageview and 
   create temporary table Landing2
    SELECT 
    website_pageviews.pageview_url,
...
FROM
    website_pageviews
        LEFT JOIN
    First_pageview ON website_pageviews.website_pageview_id= First_pageview.min_page
    where website_pageviews.pageview_url = "/home";
    
     create temporary table Bounced_session
    SELECT 
    Landing2.pageview_url,
    Landing2.website_session_id,
    COUNT(DISTINCT website_pageviews.website_pageview_id) AS Count_of_pages
FROM
    Landing2
        LEFT JOIN
    website_pageviews ON Landing2.website_session_id = website_pageviews.website_session_id
    group by 2
    having COUNT(DISTINCT website_pageviews.website_pageview_id) = 1;
    
    SELECT 
    Count(distinct Landing2.website_session_id) as sessions,
    COUNT(DISTINCt bounced_session.website_session_id) as bounced_session,
    COUNT(DISTINCt bounced_session.website_session_id)/ Count(distinct Landing2.website_session_id) as bounce_rate
    
FROM
    Landing2
        LEFT JOIN
    Bounced_session ON Landing2.website_session_id = Bounced_session.website_session_id;
    
         
         #4 bounce rate for traffic landing on the homepage
    
    select * from website_sessions;
    select * from website_pageviews;
    
## To get bounce rate we will first find the first pageview for relevant sessions, associate that pageview with url seen
## then anaysle whether that sessions had additonal pageview

##Soln
#1 Find the first  website pageview for relevant sessions
#2 identifying the landingpage of each sessions
#3 counting pageview for each sessions, to identify bounces
#4 summarizing by counting total sessions and bouncedsessions

#1 Finding the minimum website pageview assiociated with each session we care about
create temporary table First_pageviewDemo
SELECT 
...
FROM
    ...
    #2 next we will bring the landing page of each sessions
    create temporary table landing_pageDemo
    SELECT 
    First_pageviewDemo.website_session_id,
    website_pageviews.pageview_url AS Landing_page
FROM
    First_pageviewDemo
        LEFT JOIN
    website_pageviews ON First_pageviewDemo.Min_pageview_id = website_pageviews.website_pageview_id; --  Website pageview is the landing pageview

    #3 Next  we make a table to include count of pageview by session 
    create temporary table Bounced_session
    SELECT 
    Landing_pageDemo.website_session_id,
    Landing_pageDemo.Landing_page,
    COUNT( website_pageviews.website_pageview_id) AS Count_of_pages_viewed
FROM
    landing_pageDemo
        LEFT JOIN
    website_pageviews ON landing_pageDemo.website_session_id = website_pageviews.website_session_id
    group by 1,2
    ...
    #Final output
    ## we will use the same query we previously ran, and run a count of record
    ## we will group by landing page, and then we will add a bounce rate column
    
    SELECT 
    landing_pageDemo.Landing_page,
    count(distinct landing_pageDemo.website_session_id) as sessions,
    Count(distinct Bounced_session.website_session_id) AS Bounced_website_session,
     Count(distinct Bounced_session.website_session_id)/count(distinct landing_pageDemo.website_session_id) as Bounce_rate
FROM
    landing_pageDemo
        LEFT JOIN
    Bounced_session ON landing_pageDemo.website_session_id = Bounced_session.website_session_id
    group by 1;
